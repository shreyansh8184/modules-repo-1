...
class Snips(loader.Module):
    ...
    async def client_ready(self, client, db):
        ...
        client.add_event_handler(functools.partial(self.watchout),
                 events.NewMessage(outgoing=True, forwards=False))
        if db.get("SnipsMod", "state"):
            client.add_event_handler(functools.partial(self.watchout),
                 events.NewMessage(incoming=True, forwards=False))
        else:
            client.add_event_handler(functools.partial(self.watchout),
                 events.NewMessage(incoming=False, forwards=False))
    ...
    async def otherscmd(self, message):
        ...
        if state == "on":
            ...
            message.client.add_event_handler(functools.partial(self.watchout),
                 events.NewMessage(incoming=True, forwards=False))
            ...
            for key in snips:
                if argsraw == key:
                    ...
                    if not value.media and not value.web_preview:
                        ...
                        message.client.remove_event_handler(self.watchout)
                        if exec is True:
                            argspr = arg.split(" ")
                            try:
