...
class Snips(loader.Module):
    ...
    async def snipcmd(self, message):
        ...
        if not message.is_reply:
            value = await self._db.store_asset(args[1])
        else:
    ...
    async def remsnip(self, message):
        """Removes a snip from the list."""
        snipn = utils.get_args_raw(message)
        get = self._db.get("SnipsMod", "sniplist", {})
        if not snipn:
            await message.edit("<b>Please specify the name of the snip tp remove.</b>")
            return
        if snipn in get:
            del get[snipn]
    ...
    async def remsnipscmd(self, message):
        ...
        if not self._db.get("SnipsMod", "sniplist", {}):
            await utils.answer(message, "<b>There are no snips in the list to clear out.</b>")
            return
        self._db.set("SnipsMod", "snipslist", {(self._db.get("SnipsMod", "sniplist", {})).clear()})
