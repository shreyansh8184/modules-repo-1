    ...
    async def snipcmd(self, message):
        ...
        if reply and reply.media:
            if BOTLOG_CHATID:
                ...
                return
            await message.edit(
              "<code>Snip '" + snipn + "' successfully saved into the list."
            ...
            self._db.set("SnipsMod", "sniplist", sniplist)
            await message.edit(
                "<b>Snip '" + name + "' successfully saved into the list."
            ...
            if isinstance(val, int):
                ...
                try:
                    await message.client.send_message(
                        message.chat_id, loggedsnip.message,
                        reply_to=message.id,
                        file=loggedsnip.media)
                except BaseException:
                    await message.client.send_message(
                        message.chat_id, loggedsnip.message,
                        reply_to=message.id, file=None)
                return
            else:
                exec = True
                for load in self.allloaders:
