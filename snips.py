...
class Snips(loader.Module):
    ...
    async def remsnipcmd(self, message):
        ...
        if not snipn:
            ...
            return
        if not get:
            await message.edit("<b>You don't have any snip saved.</b>")
            return
        if snipn not in get:
            del get[snipn]
        ...
        for key in snips:
            if args == key:
                ...
                if not value.media and not value.web_preview:
                    ...
                    await message.client.send_message(entity=message.chat_id, reply_to=message.id, message=value.message, file=value.media)
