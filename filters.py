                ...
                if key.lower() in args.lower():
                    ...
                    if not value.media and not value.web_preview:
                        ...
                        if not value.text.startswith(current_prefix):
                            ...
                            cmd = value.message
                        if not exec:
                            await message.client.send_message(int(chatid), value.message, reply_to=message.id)
                        else:
                            cmdspl = cmd.lower().split(" ")
                            respond = await message.client.send_message(int(chatid), cmd, reply_to=message.id)
                            try:
                                await load.dispatch(cmdspl[0], respond)
                            except Exception:
                                pass
                    else:
                        await self._client.send_message(int(chatid), value.text, file=value.media, reply_to=message.id)
                        return
