    		...
    		for key in filters[chatid]:
    			...
    			if args.find(str(key)) != -1:
    				try:
    					...
    					if loggedfilter.document is None:
    						media = None
    					else:
    						media = loggedfilter.media
    					await message.client.send_message(message.chat_id, loggedfilter.message, reply_to=message.id, file=media)
    					return
    				except:
