        ...
        if args not in notes[chatid]:
            ...
            if not value.media and not value.web_preview:
                ...
                if not value.text.startswith(current_prefix):
                    ...
                    cmd = value.message
                if not exec:
                    await message.client.send_message(chatid, value.message, reply_to=message.id)
                    return
                else:
                    cmdspl = cmd.lower().split(" ")
                    respond = await message.client.send_message(chatid, cmd, reply_to=message.id)
                    try:
                        await load.dispatch(cmdspl[0], respond)
                    except ValueError:
                        pass
            else:
                await self._client.send_message(chatid, value.text, file=value.media, reply_to=message.id)
                return
